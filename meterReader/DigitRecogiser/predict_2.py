...
class NNdetect:
    ...
    def predictint(self,imvalue):
        ...
        with tf.Session() as sess:
            ...
            self.saver.restore(sess, "model2.ckpt")
    ...
    def detectDigit(self,imgName):
        """
        Main function.
        """
        imvalue = self.imageprepare(imgName)
        predint = self.predictint(imvalue)
        return (predint[0]) #first value in list 
'''
#Testing using the first argument as the target image file
#Commented out as this file in imported in the main script

if(sys.argv[1]):
    nnTest = NNdetect()
    digit =nnTest.detectDigit(sys.argv[1])
    print("DETECTED:\t"+ str(digit))
